...
> **Bloqueados (`⛔`):** `Icon` (P1.2 — **base + 4 ícones nomeados + 2/3 stories prontos**; só
> `icon--buttons` falta, depende do **Button**/P1.6) e `Tag`
> (P1.5 — usa ícones close/`Upload`/`Alert` → depende da **Icon**/P1.2). Retomar quando o
> provedor for coberto. O restante da P1 (Button/Input/List→icon; Checkbox→select/heading;
> Checkbox-v2→heading) e a P2 (Popover/Menu/Drawer) também estão acoplados. **`Stepper`
> (P4.18)** também é ⛔: o componente (não a story) importa **Button** (P1.6), **Input** (P1.7)
> e os ícones **Plus/CheckIndeterminate** (Icon/P1.2) — a classificação P4 "folha livre" valia
> só pelos imports da story; o componente tem dependências. Retomar após Button/Input/Icon.
...
**Placar:** `✅ 14` · `⚠️ 51` (sendo `Icon`, `Tag` e `Stepper` ⛔ bloqueados) · `🚫 (fora do escopo) 7`
...
### Icon — `icon` — ⚠️ Parcial (2/3, 2026-06-08) — falta `icon--buttons` (Button)

- **Stories (clone/orig):** 2/3 — `attributes` ✓, `overrides` ✓; **`buttons` adiada** (depende de Button).
...
- **Pendente p/ ✅:** clonar `icon--buttons` após o Button (P1.6).
- **Commit:** _pendente_.
