...
> **Estado atual:** **`Pagination`, `Accordion`, `Typography`, `Spinner`, `Avatar` e `Divider`
> cobertos**. Todos os demais estão `⚠️ Divergente` (faltam stories e/ou verificação).
...
> Checkbox-v2→heading) e a P2 (Popover/Menu/Drawer) também estão acoplados.
...
**Placar:** `✅ 6` · `⚠️ 59` (sendo `Icon` e `Tag` ⛔ bloqueados) · `🚫 (fora do escopo) 7`
