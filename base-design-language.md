...
> **Icon (P1.2) ✅ e Button (P1.6) ✅ cobertos** — destravaram `Tag`, `Input`, `List`, `Badge`,
> `Breadcrumbs`, `Progress steps`. **Bloqueado (`⛔`):** só **`Stepper` (P4.18)** — o componente
> (não a story) importa **Button** (✅), **Input** (P1.7 ⚠️) e ícones (✅); falta só o **Input**.
> O restante da P1 (Checkbox→select/heading; Checkbox-v2→heading) e a P2 (Popover/Menu/Drawer)
> seguem acoplados aos seus provedores.
...
**Placar:** `✅ 16` · `⚠️ 49` (`Input` 7/15 core verificado; `Stepper` ⛔ agora só aguarda o resto do Input) · `🚫 (fora do escopo) 7`
...
### Input — `input` — ⚠️ Parcial (7/15, 2026-06-08) — core verificado

- **Stories (clone/orig):** 7/15 — `input`, `sizes`, `states`, `before-after`, `clearable`,
  `password`, `with-button` (verificadas). **Pendentes:** `mask`/`number`/`selector` (variantes de
  tipo), `form-control-states` (depende de **FormControl** 🚫), `disabled-matches-select` (depende
  de **Select**), `*-icon-overrides`/`clearable-noescape` (API de overrides/comportamento).
...
- **Verificação (orig→clone):** CSS computado **idêntico** — `sizes` heights 32/36/48/60, borda 2px
  ...
  **error** `rgb(222,17,53)`; `password` mascarado + toggle olho; `clearable` ✕; `with-button`
  input+botão (36/48). **AXE:** resíduo (`label`/disabled `color-contrast`) **idêntico ao original**
