...
> `Heading`, `Link`, `Skeleton`, `Aspect ratio box`, `Progress bar`, `Rating`, `Switch`, `Tabs`
> e `List` cobertos**. Todos os demais estão `⚠️ Divergente` (faltam stories e/ou verificação).
> **Icon (P1.2) ✅, Button (P1.6) ✅, Input (P1.7, 9/15 core), Stepper (P4.18) ✅ e List (P1.10) ✅ cobertos.**
> **Nenhum bloqueio (`⛔`) restante.** `Tag` livre (depende só de Icon). Com `List` fechado, o único
> P1 restante são `Checkbox` (P1.8→select/heading) e `Checkbox-v2` (P1.9→heading); a P2
> (Popover/Menu/Drawer) segue acoplada aos seus provedores.
...
**Placar:** `✅ 19` · `⚠️ 44` (nenhum ⛔ — Input 9/15 core) · `🚫 (fora do escopo)`
