...
import { ShopWindowItem } from '@shared/interfaces/entity/shop-window';
...
export class ShowcasePageComponent {
  ...
  readonly relatedItems = signal<ShowcaseRelatedItem[]>([]);
  ...
  readonly filteredRelated = computed(() => {
    ...
    return this.relatedItems()
      .filter((r) => r.titleLink !== current?.media.titleLink)
      .slice(0, 20);
  });
  ...
  private loadData(username: string, titleLink: string): void {
    forkJoin({
      ...
      shopWindows: this.shopWindowApi.list(1, 20),
    }).subscribe({
      next: ({ post, shopWindows }) => {
        if (!post.data) {
          this.hasError.set(true);
          return;
        }
        this.post.set(post.data);
        const flat: ShowcaseRelatedItem[] = (shopWindows.data?.data ?? []).flatMap((sw) =>
          sw.items.map((item) => ({
            ...item,
            username: sw.channel.profileNameOfficial,
          })),
        );
        this.relatedItems.set(flat);
      },
